/**
 * Pure data helpers for the attribute table Charts dialog: numeric-column
 * detection, histogram binning, categorical aggregation and scatter/line/box
 * extraction. Kept free of any rendering or React so they can be unit-tested in
 * isolation; the SVG drawing lives in the chart components.
 *
 * They operate on the same `{ properties }` rows the attribute table builds for
...
/**
 * Parse a value into a finite number, or undefined when it cannot be one.
 * Numeric strings ("42", " 3.5 ") are accepted; empty/blank, boolean, null, NaN
 * and Infinity are rejected so they never enter a chart.
 */
 ...
 * sources (eg. CSV, or GeoJSON properties read through TableMixin) adapt their
 * analysis rows with {@link coerceNumericStringRows} first.
 */
...
/** True for common field names that conventionally hold identifiers. */
function isIdentifierFieldName(key: string): boolean {
  const normalized = key.replace(/([a-z\d])([A-Z])/g, "$1_$2").toLowerCase();
  if (
    /(^|[\s_-])(id|fid|code|codice|istat|cap|zip|zipcode|postal)([\s_-]|$)/.test(
      normalized
    )
  ) {
    return true;
  ...
  return (
    ...
    /^istat(com|prov|reg)\d*$/.test(compact)
  );
}

/**
 * Convert numeric-looking strings in analysis rows without changing the source
 * data. Most catalog items store every property as text (GeoJSON read through
 * TableMixin, CSV, ...) even though summaries should treat measurements as
 * numeric. Since those values carry no declared type, common identifier field
 * names and integer strings with leading zeroes remain text - a postcode or an
 * ISTAT municipality code must not become a number.
 *
...
export function coerceNumericStringRows(rows: ChartRow[]): ChartRow[] {
  ...
  numericCounts.forEach((numeric, key) => {
...
export function pickAnalysisRows(
  ...
  return analysisRows.filter((_row, index) =>
...
/**
 * Bin a set of values into `binCount` equal-width buckets. Returns undefined
 * when there are no values. When every value is identical (min === max) a single
 * bin holding them all is returned, avoiding a zero-width divide.
 */
export function computeHistogram(
  values: number[],
  binCount: number
): HistogramResult | undefined {
  ...
  const bins: HistogramBin[] = new Array(count).fill(0).map((_v, i) => ({
    x0: min + i * width,
...
export interface ScatterResult {
  /** Points to render - an evenly strided sample capped at MAX_SCATTER_POINTS. */
  points: ScatterPoint[];
...
/**
 * Extract the (x, y) pairs where both columns hold a finite number. Returns
 * undefined when no row has both. Extents span every valid pair. When there are
 * more than `maxPoints`, `points` is an **evenly strided** subset (not the
 * leading rows) so a huge - and often spatially-ordered - layer renders a
 * representative sample rather than just its first features; `total` reports the
 * full count.
 */
export function computeScatter(
  ...
  if (all.length > maxPoints) {
    ...
    points = all.filter((_point, index) => index % stride === 0);
  }
 ...
 * decimals with trailing zeros trimmed. Large/small magnitudes fall back to
 * exponential so labels stay short.
 */
export function formatAxisValue(value: number): string {
  if (!Number.isFinite(value)) return "";
  const abs = Math.abs(value);
  // Exponential for extreme magnitudes - checked before the integer path so a
  // huge integer (eg. 9e15) doesn't print 16 digits and overflow the label.
 ...
 * between one and `maxCardinality` distinct entries AND are repetitive enough to
 * be a category rather than an id (see the limits above). Low-cardinality
 * numeric codes (eg. a year or class id) qualify; a continuous numeric field or
 * a unique-per-row text column does not.
 */
 ...
 * fields first so the automatic selection remains useful. Unique labels such as
 * names and IDs stay available for explicitly configured charts.
 */
export function categoryColumnOptions(
  ...
  return [
    ...preferred,
    ...columns.filter((column) => !preferredSet.has(column))
  ];
}
...
/** Label used for rows whose category value is null or blank. */
export const BLANK_CATEGORY_LABEL = "(blank)";
 ...
 * undefined when there are no rows, or - for `sum`/`mean` - when no category has
 * any numeric value to aggregate (those categories are dropped rather than shown
...
export function computeBar(
  ...
  const all: BarDatum[] = Array.from(groups.entries())
 ...
 * dropped. Null/blank category values bucket as "(blank)". Returns undefined
 * when no positive slice survives.
 */
...
/**
 * The finite values of a numeric field plotted against original row order. Rows
 * without a finite value are skipped (leaving a gap), keeping each point's x at
 * its true feature index. Returns undefined when no row has a value.
 */
 ...
 * undefined when empty. Sorts a copy of all values synchronously - intended for
 * the in-memory feature sets the attribute table holds.
 */
export function computeBox(values: number[]): BoxResult | undefined {
  if (values.length === 0) return undefined;
