/**
 * Pure row-selection logic for the attribute table. Kept free of React/MobX
...
export interface RowSelectionInput {
  ...
  selectedIds: string[];
  /** The current anchor (last primary pick), or undefined when nothing is selected. */
...
export interface RowSelectionResult {
  ...
  ids: string[];
  /** The next anchor (always a member of `ids`, or undefined when `ids` is empty). */
 ...
 *   over `sortedIds`, keeping the anchor fixed so repeated Shift-clicks
...
export function computeRowSelection(
  ...
  if (range) {
    const anchorIndex =
      anchorId !== undefined ? sortedIds.indexOf(anchorId) : -1;
    ...
    return { ids: Array.from(new Set(merged)), anchor: anchorId };
  }
  ...
  if (additive) {
    ...
    const anchor = next.includes(featureId)
      ? featureId
      : anchorId !== undefined && next.includes(anchorId)
      ? anchorId
      : next[next.length - 1];
