/**
 * Pure data helpers for the attribute table's field-statistics summary: detect
 * whether a field reads as numeric or text and compute a compact summary for it
 * (count / nulls / min / max / mean / median / std / sum / unique for numbers;
 * count / nulls / unique / most-frequent values for text). Kept free of any
 * rendering or React so they can be unit-tested in isolation.
 *
 * Field type detection respects the primitive types stored in `properties`, so a
 * text field is not reclassified just because every string parses as a number.
 * String-only sources (CSV, GeoJSON read through TableMixin) run their rows
 * through `coerceNumericStringRows` first.
 */
...
export interface NumericFieldStats {
  ...
  nulls: number;
  /** Non-null rows whose value is not a finite number (eg. stray text). */
  nonNumeric: number;
 ...
 * stays a pure reduction over the values it was handed. Returns undefined only
 * when there are no values to summarize.
 */
export function computeNumericStats(
  values: number[],
  nulls = 0,
  nonNumeric = 0
): NumericFieldStats | undefined {
  ...
  // Sample standard deviation (divide by n - 1, matching Excel's STDEV);
 ...
 * of the populated rows. Numeric-looking strings therefore remain text unless
 * the caller adapts a string-only source before summarizing it. Numeric fields
 * fold their blank and non-numeric row counts into the result.
 */
 ...
 * the readout stays compact. Mirrors the chart axis formatting but keeps a
 * little more precision (means/std are rarely whole numbers).
 */
 ...
 * user keeps reading a subset instead of being dropped back onto the whole layer
 * without asking. Only when neither narrows the layer does it fall to "all".
 * Callers render from the result rather than from the picked scope, so a scope
 * that disappears cannot leave a stale label on the summary or its clipboard
 * header.
 *
