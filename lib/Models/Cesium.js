...
Cesium.prototype.zoomTo = function(target, flightDurationSeconds) {
  ...
  flightDurationSeconds = defaultValue(flightDurationSeconds, 5.0);
...
function zoomToDataSource(cesium, target, flightDurationSeconds) {
  return pollToPromise(
    function() {
      ...
      var boundingSphere;
      // Enlarge rect if dataSource has only a single point.
      if(entities.length == 1 && typeof entities[0].position !== 'undefined' && entities[0].position.length == 1) {
        const margin = Cartesian3.fromElements(750, 750, 0);
        
        const entityPosition = entities[0].position.getValue();
        let p1 = new Cartesian3();
        Cartesian3.subtract(entityPosition, margin, p1);
        let p2 = new Cartesian3();
        Cartesian3.add(entityPosition, margin, p2);

        cesium.scene.camera.flyTo({
          destination : Rectangle.fromCartesianArray([p1, p2], cesium.scene.globe.ellipsoid)
        });
      }
      else {
...
Cesium.prototype.pickFromScreenPosition = function(
  ...
  this.terria.isFeaturePicked = true;
};
