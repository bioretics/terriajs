  ...
  return pollToPromise(
    function() {
      ...
      if(entities.length == 1) {
        ...
        const ccc = Cartesian3.fromElements(750, 750, 0);
        const aaa = entities[0].position.getValue();
        let bbb = new Cartesian3();
        Cartesian3.subtract(aaa, ccc, bbb);
        let ddd = new Cartesian3();
        Cartesian3.add(aaa, ccc, ddd);

        cesium.scene.camera.flyTo({
          destination : Rectangle.fromCartesianArray([bbb, ddd], cesium.scene.globe.ellipsoid)
        });
      }
      else {
