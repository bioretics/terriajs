...
var Cesium = function(terria, viewer) {
  ...
  this.eventHelper.add(this.scene.renderError, function(scene, error) {
      this.terria.error.raiseEvent(new TerriaError({
          sender: this,
          title: 'Errore di rendering in 3D',
          message: '\
<p>Errore durante il rendering 3D. Questo probabilmente indica un bug in ' + terria.appName + ' o incompatibilità con il sistema \
o il browser. Viere ora attivata la modalità 2D. Segnalare il problema scrivendo a  <a href="mailto:' + terria.supportEmail + '">' + terria.supportEmail + '</a> \
riportando i seguenti dettagli tecnici. Grazie!</p><pre>' + formatError(error) + '</pre>'
      }));
...
Cesium.prototype.pickFromScreenPosition = function(screenPosition) {
  ...
  var pickRasterPromise = this.scene.imageryLayers.pickImageryLayerFeatures(
    pickRay,
    this.scene
  );
