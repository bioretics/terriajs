    ...
    return when(that.data, function(data) {
      if (typeof Blob !== "undefined" && data instanceof Blob) {
        return readText(data).then(function(text) {
          return loadTableFromCsv(that, text).then((resolve) => {
            if(typeof that.dataSource !== 'undefined' && that.dataSource.entities !== 'undefined' && that.dataSource.entities.values !== 'undefined') {
              that.dataSource.entities.values.forEach((elem) => {
                elem.point.heightReference = HeightReference.CLAMP_TO_GROUND;
              });
            }
            return resolve;
          });
