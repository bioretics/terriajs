...
GeoJsonCatalogItem.prototype.canUseAsPath = function() {
  let pathType = undefined;

  if(this.data && this.data.crs.type === "EPSG" && this.data.crs.properties.code === "4326") {
    if(this.data.type === "FeatureCollection" && this.data.features && this.data.features.length == 1 && this.data.features) {
      const geometry = this.data.features[0].geometry;
    ...
    else if(this.data.type === "LineString" && this.data.coordinates.length > 1) {
      pathType = PathTypes.lineString;
    }
    else if(this.data.type === "MultiLineString" && this.data.coordinates.length == 1) {
      pathType = PathTypes.multiLineString;
    }
    /*else if(this.data.type === "GeometryCollection") {
...
GeoJsonCatalogItem.prototype.useAsPath = function() {
  let coordinates;

  switch(this.pathType) {
    case PathTypes.featureCollectionMultiLineString:
      coordinates = this.data.features[0].geometry.coordinates[0];
      break;
    case PathTypes.featureCollectionLineString:
      coordinates = this.data.features[0].geometry.coordinates;
      break;
  ...
  if(coordinates.length > 0) {
    ...
    const results = preparePath(coordinates);
...
function loadGeoJson(geoJsonItem) {
  ...
  var options = {
    ...
    strokeWidth: defaultValue(style["stroke-width"], 5),
    ...
    clampToGround: true, //geoJsonItem.clampToGround,
    ...
    for (var i = 0; i < entities.length; ++i) {
      ...
      if (
        ...
        entity.point = new PointGraphics({
          ...
          //TODO: options.clampToGround
          heightReference: HeightReference.CLAMP_TO_GROUND /*options.clampToGround
...
function preparePath(coordinates, ellipsoid) {
  const positions = [];
