            ...
            if (defined(layer) && (layer.show === true || !defined(layer.show))) {
                ...
                if (catalogItem === catalogItem.terria.baseMap ||
                    ...
                    globeOrMap.terria.error.raiseEvent(new TerriaError({
                        sender: catalogItem,
                        title: 'Errore di accesso alla mappa base',
                        message: '\
Errore rilevato durante il tentativo di scaricare i tiles per la mappa base ' + catalogItem.terria.baseMap.name + '. Questo può indicare un problema con la connessione Internet, \
che la mappa base è temporaneamente irraggiungibile o che non è valida. Selezionare una differente mappa base clickando sul bottone "Mappe base". <br/><br/>\
<pre>' + formatError(e) + '</pre>'
                    ...
                    globeOrMap.terria.error.raiseEvent(new TerriaError({
                        sender: catalogItem,
                        title: 'Errore di accesso a un elemento del catalogo',
                        message: '\
Errore rilevato durante il tentativo di scaricare i tiles per il layer ' + catalogItem.terria.baseMap.name + '. Questo può indicare un problema con la connessione Internet, \
che il layer è temporaneamente irraggiungibile o che non è valido. Il layer è stato spento sulla mappa, è possibile riprovare ad accenderlo dal pannello "Attivi ora". <br/><br/>\
<pre>' + formatError(e) + '</pre>'
                    }));
        ...
        onProjectionError: function() {
            ...
            globeOrMap.terria.error.raiseEvent({
                ...
                message: '\
"' + catalogItem.name + '" non può essere mostrato in 2D perchè non supporta la proiezione standard Web Mercator (EPSG:3857).  \
Tornare al 3D, aggiornare il layer perchè supporti la proiezione o cambiare layer.'
            });
