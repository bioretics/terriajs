  ...
  if (!catalogItem.supportsIntervals) {
    throw new TerriaError({
      ...
      message:
        ...
        " (" +
        catalogItem.type +
        ') catalog items cannot currently be made time-varying by specifying the "intervals" property.'
    });
  }
...
ImageryLayerCatalogItem.enableLayer = function(
  ...
  const layer = globeOrMap.addImageryProvider({
          ...
          if (
            ...
            globeOrMap.terria.error.raiseEvent(
              new TerriaError({
                ...
                message:
                  "An error occurred while attempting to download tiles for base map:<p><center>" +
                  catalogItem.terria.baseMap.name +
                  "</center></p> This may indicate that there is a " +
            ...
            globeOrMap.terria.error.raiseEvent(
              new TerriaError({
                ...
                message:
                  "An error occurred while attempting to download tiles for catalogue item:<p><center>" +
                  catalogItem.name +
                  "</center></p> This may indicate that there is a " +
      ...
      operation.attempt(function() {
        ...
        maybeXhr
          .then(function() {
            ...
            const key = `L${tileProviderError.level}X${tileProviderError.x}Y${
              tileProviderError.y
            }`;
            ...
            if (++tileRetriesByMap[key] > 5) {
              failTile({
                ...
                message:
                  ...
                  "\n" +
                  "viene caricata con successo con una XMLHttpRequest ma ha fallito più volte se caricata direttamente da Cesium o Leaflet."
              });
            } else {
              // Either:
          ...
          .otherwise(function(e) {
            ...
            if (e.statusCode >= 400 && e.statusCode < 500) {
              ...
              failTile({
                name: "Error di Tile",
                message:
                  "Ricevuta una risposta 200 (OK) dall'URL:\n" +
                  getUrlForImageryTile(
                    imageryProvider,
                    tileProviderError.x,
                    tileProviderError.y,
                  ...
                  "\n" +
                  "ma l'immagine non può essere caricata."
              });
              ...
              if (catalogItem.ignoreUnknownTileErrors) {
                ...
                failTile({
                  name: "Errore di Tile sconosciuto",
                  message:
                    "Tentando di accedere all'URL:\n" +
                    getUrlForImageryTile(
                      imageryProvider,
                      tileProviderError.x,
                      tileProviderError.y,
                    ...
                    "\n" +
                    "Questo può indicare:\n" +
                    "* l'hostname del server non viene risolto correttamente,\n" +
                    "* il server non risponde alla richiesta, o\n" +
                    "* il server nega l'accesso CORS (Cross-Origin Resource Sharing), vedi https://docs.terria.io/guide/connecting-to-data/cross-origin-resource-sharing/."
                });
              }
            } else {
    ...
    onProjectionError: function() {
      ...
      globeOrMap.terria.error.raiseEvent({
        sender: catalogItem,
        title: "Impossibile mostrare il layer",
        message:
          catalogItem.name +
          '" non può essere mostrato in 2D perchè non supporta la proiezione standard Web Mercator (EPSG:3857).  ' +
          "Passare alla modalità 3D se supportata dal sistema, aggiornare il layer perchè supporti la proiezione o usare un diverso layer."
      });
  ...
  if (!defined(featureTimes)) {
    throw new TerriaError({
      ...
      message:
        `${
          this.name
        } cannot be shown at this location because the available capture times are unknown. ` +
