...
var OpenStreetMapCatalogItem = function(terria) {
     ...
     */
     ...
     /**
     * Gets or sets the tag for API that require authentication
     * This property is observable.
     * @type {String}
     */
     this.keyTag = 'apikey';
    ...
    /**
     * Gets or sets the key for API that require authentication
     * This property is observable.
     * @type {String}
     */
     this.key = undefined;

    knockout.track(this, ['fileExtension', 'maximumLevel', 'attribution', 'keyTag', 'key']);
};
...
function cleanAndProxyUrl(catalogItem, url) {
    var uri;
    if (defined(catalogItem.key)) {
        uri = cleanUrlAndAddKey(url, catalogItem.keyTag, catalogItem.key);
    }
    else {
        uri = cleanUrl(url);
    }
    return proxyCatalogItemUrl(catalogItem, uri);
}
...
function cleanUrlAndAddKey(url, tag, key) {
    // Add a search option with API KEY
    var uri = new URI(url);
    uri.search('').search({ apikey: key });
    return uri.toString();
}
