...
var RerFeatureServerCatalogItem = function(terria) {
   ...
   * @type {Number}
   */
  this.lowPitchAngle = 30;

  /**
   * 
   * @type {Number}
   */
  this.veryLowPitchAngle = 15;
...
RerFeatureServerCatalogItem.prototype._load = function() {
  ...
  this._geoJsonItem.distanceFilter = this.distanceFilter;
  this._geoJsonItem.cacheFeatures = this.cacheFeatures;
  this._geoJsonItem.labels = this.labels;
...
function checkReload(item) {
  ...
  if(camera.positionCartographic.height !== item.oldHeight) {
    ...
    let scale = 22.56994;
    let level;
    for(level = 19; level > item.levelMin && scaleDivider > scale; --level) {
      scale *= 2;
    }
    item.oldHeight = camera.positionCartographic.height;
    if(level > item.levelMax)
      level = item.levelMax;
      if(item.level !== level /*&& level >= item.levelMin*/) {
        item.level = level;
        changed = true;
    }
  }
  ...
  /*if(!changed || Math.abs(item.level - item.oldLevel) > 1)*/ {
    if(camera.pitch >= -CesiumMath.toRadians(item.lowPitchAngle)) {
      item.cameraPitch = item.PITCH_LOW;
      //let polygonHeight = item.lowPitchFirstHeightSplit;

      if(camera.pitch >= -CesiumMath.toRadians(item.veryLowPitchAngle)) {
        if(item.distanceFilter.hideFeaturesOnVeryLowPitch) {
          return;
        }
...
function loadGeoJson(item) {
  return loadJson(buildGeoJsonUrl(item)).then(function(json) {
    ...
    jsonRes.features.forEach(elem => {
      const iconClass = item.icons ? item.icons.classes[elem.properties[item.icons.field]] : {"marker-symbol": "marker"};
      Object.assign(elem.properties, iconClass);
    });
    return jsonRes
  });
}
