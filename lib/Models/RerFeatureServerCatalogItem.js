  ...
  if (defined(this._geoJsonItem)) {
    ...
    if(this.terria.currentViewer.terria.cesium) {
      ...
      this.removeMoveEndEventListener = this.terria.currentViewer.terria.cesium.viewer.scene.camera.moveEnd.addEventListener(function() {
        ...
        var newBBox = new Rectangle(
          CesiumMath.toDegrees(viewRect.west - viewRect.west * 0.0005),
          CesiumMath.toDegrees(viewRect.south - viewRect.south * 0.0005),
          CesiumMath.toDegrees(viewRect.east + viewRect.east * 0.0005),
          CesiumMath.toDegrees(viewRect.north + viewRect.north * 0.0005));
        ...
        if(typeof pickedTriangle !== "undefined") {
          ...
          if(tmpLevel != that.level || !newBBox.equalsEpsilon(that.bbox, 0.0001)) {
            that.bbox = newBBox;
...
function buildGeoJsonUrl(item) {
  ...
  var url = new URI(url)
    ...
    .addQuery(
      "geometry", item.bbox.west + "," + item.bbox.south + "," + item.bbox.east + "," + item.bbox.north
    )
    .addQuery(
