...
var ConstantPositionProperty = require("terriajs-cesium/Source/DataSources/ConstantPositionProperty");
var BoxGraphics = require("terriajs-cesium/Source/DataSources/BoxGraphics");
...
RerGeoJsonCatalogItem.prototype.filter = function(cameraPosition, pitchIsLow) {
  //console.log(this._dataSource.entities.values);

  var count = 0;
  var distMax = 0;

  /*this._dataSource.entities.values.forEach(element => {
    //element.billboard.scale = 10;
    if(pitchIsLow) {
      var distance = Cartesian3.distance(element.position.getValue(), cameraPosition);
      if(distance > 30000) {
        element.show = false;
  ...
  //console.log(this._dataSource.entities.values.length);
  //var tmp = ;
  //console.log(tmp);
  /*this._dataSource.entities.add({
    ...
    name: "urcaaa",
    show: true,
    postion: Cartesian3.fromDegrees(-114.0, 40.0, 300000.0),
    box: {
  ...
  //this._dataSource.entities.values[0].billboard.color = Color.YELLOW;
  //console.log(this._dataSource.entities.values[0].position.getValue());

  
  //console.log("COUNT: " + count.toString() + " with " + pitchIsLow + " and distMax = " + distMax.toString());
};
