...
var mergeSort = require("terriajs-cesium/Source/Core/mergeSort").default;
 ...
 */
var RerGeoJsonCatalogItem = function(terria, url) {
  ...
  this.level = 0;
  this.cameraPitch = 0;
  this.windowHeightToRay = 0.7
  this.cacheFeatures = true;
  /*this.filterFeaturesByDistancePerLevel = undefined;
  this.showOnlyNearestFeaturesPerLevel = undefined;
  this.showLabelByDistancePerLevel = undefined;
  this.hideFeaturesOnVeryLowPitch = false;
  this.labelFont = undefined;
  this.labelFillColor = undefined;
  this.backgroundColor = undefined;*/
  this.distanceFilter = undefined;
  this.labels = undefined;
};

inherit(GeoJsonCatalogItem, RerGeoJsonCatalogItem);
...
RerGeoJsonCatalogItem.prototype._load = function() {
  var codeSplitDeferred = when.defer();

  const showOnlyNearest = this.distanceFilter && this.distanceFilter.nearestPerLevel && this.distanceFilter.nearestPerLevel.isEnabled;
  const filterByDistance = this.distanceFilter && this.distanceFilter.displayDistanceMaxPerLevel && this.distanceFilter.displayDistanceMaxPerLevel.isEnabled;
  const showLabelByDistance = this.labels && this.labels.classes;
...
at <a href="mailto:' +
            ...
            return when(promise, function(json) {
              if(!that.terria.cesium || !that.terria.cesium.scene) {
                return;
              }
              const newJson = {
                type: json.type,
              ...
              if(showOnlyNearest && that.cameraPitch > 0) {
                for(let i = 7; i <= 16; ++i) {
                  tmpElements.push([]);
                }
              }
              ...
              if(that.cacheFeatures) {
                for(let feat of json.features) {
                  if(!that.dataCache.has(feat.id)) {
                    that.dataCache.set(feat.id, feat);
                  }
                }
              }
              else {
                that.dataCache = json.features;
              }

              that.dataCache.forEach((value, key, map) => {
                const lvl = parseInt(value.properties["LEVEL_ID"]);
                if(lvl <= that.level) {
                  value.properties['show_label'] = true;
                  if(that.cameraPitch > 0) {
                    if(typeof value.positionCartesian2 === 'undefined') {
                      const positionCartographic = Cartographic.fromDegrees(
                        value.geometry.coordinates[0], 
                        value.geometry.coordinates[1], 
                        scene.camera.positionCartographic.height
                      );
                      value.positionCartesian2 = Cartesian2.fromCartesian3(Cartographic.toCartesian(positionCartographic, scene.globe.ellipsoid));
                    }

                    if(filterByDistance || showOnlyNearest || showLabelByDistance) {
                      const width = scene.canvas.clientWidth;
                      const height = scene.canvas.clientHeight;
                      const ray = scene.camera.getPickRay(new Cartesian2(width * 0.5, height * that.windowHeightToRay));
                      value.dist = Cartesian2.distanceSquared(value.positionCartesian2, Cartesian2.fromCartesian3(scene.globe.pick(ray, scene)));

                      if(showLabelByDistance) {
                        const labelClass = typeof(that.labels.classPerLevel) === "number" ? that.labels.classes[that.labels.classPerLevel] : that.labels.classes[that.labels.classPerLevel[properties['LEVEL_ID'] - 7]];
                        const show = Object.keys(labelClass).length !== 0;
                        if(show && labelClass.displayDistanceMax > 0) {
                          if(value.dist > labelClass.displayDistanceMax * labelClass.displayDistanceMax) {
                            value.properties['show_label'] = false;
                          }
                        }
                      }
                      if(filterByDistance) {
                        const distanceThreshold = that.cameraPitch == 1 ? 
                          that.distanceFilter.displayDistanceMaxPerLevel.distances[lvl - 7] : 
                          that.distanceFilter.displayDistanceMaxPerLevel.veryLowPitchDistances[lvl - 7];
                        if(value.dist > distanceThreshold * distanceThreshold) {
                          return;
                        }
                      }
                      else if (showOnlyNearest) {
                        tmpElements[lvl - 7].push(value);
                        return;
                      }
                    }
                  }
              ...
              if(showOnlyNearest && that.cameraPitch > 0) {
                for(let i = 0; i < tmpElements.length; ++i) {
                  mergeSort(tmpElements[i], function(a, b) {
                    return a.dist - b.dist;
                  });
                  for(let j = 0; j < that.distanceFilter.nearestPerLevel.nearest[i] && j < tmpElements[i].length; ++j)
                  {
                    newJson.features.push(tmpElements[i][j]);
                  }
                }
              }
              else {
                newJson.features = tmpElements;
              }

              console.log("RerGeoJsonCatalogItem LOAD: level = " + that.level.toString() 
                + (that.cameraPitch === 0 ? "" : (that.cameraPitch === 1 ? " - pitchIsLow" : " - pitchIsVeryLow")) 
                + " => features = " + newJson.features.length.toString() + "/" + (that.cacheFeatures ? that.dataCache.size : that.dataCache.length).toString());

              that.data = newJson;
              return updateModelFromData(that, newJson);
            }).otherwise(function() {
              throw new TerriaError({
                sender: that,
                title: "Error loading GeoJSON",
...
function loadGeoJson(geoJsonItem) {
  ...
  /*const nfScalar = new NearFarScalar(1000, 2, 10000, 0.5);
  const ddCondition = new DistanceDisplayCondition(0, 12000);*/
  ...
  return dataSource.load(geoJsonItem._readyData, options).then(function() {
    ...
    for (var i = 0; i < entities.length; ++i) {
      ...
      const labelClass = typeof(geoJsonItem.labels.classPerLevel) === "number" ? geoJsonItem.labels.classes[geoJsonItem.labels.classPerLevel] : geoJsonItem.labels.classes[geoJsonItem.labels.classPerLevel[properties['LEVEL_ID'] - 7]];
      const show = Object.keys(labelClass).length !== 0;

      if(show && properties['show_label'].getValue()) {
        entity.label = new LabelGraphics({
          text: properties['NOME'], 
          font: labelClass.font, 
          fillColor: Color.fromCssColorString(labelClass.fillColor),
          backgroundColor: Color.fromCssColorString(labelClass.backgroundColor),
          showBackground: typeof labelClass.backgroundColor !== 'undefined' && labelClass.backgroundColor !== "transparent" ? true : false,
      ...
      /*entity.billboard.scaleByDistance = nfScalar;
      entity.billboard.distanceDisplayCondition = ddCondition;
      entity.label.scaleByDistance = nfScalar;
      entity.label.distanceDisplayCondition = ddCondition;*/
...
module.exports = RerGeoJsonCatalogItem;
