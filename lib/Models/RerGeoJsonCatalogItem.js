...
var RerGeoJsonCatalogItem = function(terria, url) {
  ...
  this.filterDistances = undefined;
  this.filterFeaturesByDistance = false;
...
at <a href="mailto:' +
            ...
            return when(promise, function(json) {
              ...
              that.dataCache.forEach((value, key, map) => {
                ...
                if(lvl <= that.level) {
                  if(that.cameraPitch > 0) {
                    ...
                    if(that.filterFeaturesByDistance || that.showOnlyNearestFeatures) {
                      ...
                      if(that.filterFeaturesByDistance) {
                        const distanceThreshold = that.filterDistances[lvl - 7][that.cameraPitch - 1];
              ...
              if(that.showOnlyNearestFeatures > 0 && that.cameraPitch > 0) {
                mergeSort(tmpElements, function(a, b) {
                  return a.dist - b.dist;
                });
                for(let i = 0; i < that.showOnlyNearestFeatures && i < tmpElements.length; ++i)
                {
                  newJson.features.push(tmpElements[i]);
                }
              }
              else {
  ...
  return dataSource.load(geoJsonItem._readyData, options).then(function() {
    ...
    for (var i = 0; i < entities.length; ++i) {
      ...
      entity.label = new LabelGraphics({
        ...
        font: geoJsonItem.labelFont, 
        fillColor: Color.fromCssColorString(geoJsonItem.labelFillColor),
        backgroundColor: Color.fromCssColorString(geoJsonItem.backgroundColor),
        showBackground: typeof geoJsonItem.backgroundColor !== 'undefined' && geoJsonItem.backgroundColor !== "transparent" ? true : false,
