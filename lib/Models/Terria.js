...
var Terria = function(options) {
     ...
     * @type {Array}
     */
    this.elevationPoints = undefined;

    /**
     * 
     * @type {Array}
     */
    this.stepDistanceMetres = undefined;
    ...
    knockout.track(this, [
        ...
        'baseMapContrastColor', 'elevationPoints', 'stepDistanceMetres'
    ]);
...
function loadInitSource(terria, source) {
    ...
    if (typeof source === 'string') {
        ...
        if (fragmentPaths.length === 0) {
            terria.error.raiseEvent({
                title: 'Errore nel caricamento del file di inizializzazione',
                message: 'Non è possibile caricare da ' + fragment + '.'
            });
            return undefined;
        }
    ...
    if (loadPromise) {
        ...
        .otherwise(function () {
            terria.error.raiseEvent({
                title: 'Errore nel caricamento del file di inizializzazione',
                message: 'Errore nel caricamento del file di inizializzazione ' + loadSource + '.  Questo può essere dovuto ad un link non valido o ad un problema di connessione.'
            });
            return undefined;
        });
    } else {
