...
var Terria = function(options) {
  ...
  /**
   * Array of step of path distance
   * @type {Array}
   */
  this.stepDistanceMeters = undefined;
  ...
  if (typeof source === "string") {
    ...
    if (fragmentPaths.length === 0) {
      terria.error.raiseEvent({
        ...
        message:
          "Non è possibile caricare da " +
          fragment +
          " perchè non ci sono initFragmentPaths definiti."
      });
      return undefined;
    }
    ...
    return loadPromise
      ...
      .otherwise(function() {
        terria.error.raiseEvent({
          title: "Errore nel caricamento del file di inizializzazione",
          message:
            "Errore nel caricamento del file di inizializzazione " +
            loadSource +
            ". Questo può essere dovuto ad un link non valido o ad un problema di connessione."
        });
        return undefined;
      });
  } else {
