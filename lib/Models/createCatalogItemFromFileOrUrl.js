...
var createCatalogItemFromFileOrUrl = function(terria, viewState, fileOrUrl, dataType, confirmConversion) {
    function tryConversionService(newItem) {
        if (terria.configParameters.conversionServiceBaseUrl === false) {
            ...
            terria.error.raiseEvent(new TerriaError({
                ...
                message: 'Questo formato di file non è supportato da ' + terria.appName + '. I formati supportati comprendono: ' +
            ...
            terria.error.raiseEvent(new TerriaError({
                ...
                message: 'Questo formato di file non è supportato da ' + terria.appName + '. I formati supportati comprendono: ' +
                '<ul><li>.geojson</li><li>.kml, .kmz</li><li>.csv (in <a href="https://github.com/TerriaJS/nationalmap/wiki/csv-geo-au">csv-geo-au format</a>)</li></ul>' +
                'Formati supportati attraverso il servizio di conversione: ' +
                '<ul><li>Shapefile (SOLO SE COMPRESSI .zip)</li><li>MapInfo TAB (SOLO SE COMPRESSI .zip)</li><li>Altri <a href="http://www.gdal.org/ogr_formats.html">OGR Vector Formats</a></li></ul>'
            }));
            return undefined;
        }
        return getConfirmation(terria, viewState, confirmConversion, 'Questo formato di file non è supportato da '+terria.appName+'.\n\n' +
            'Tentare la conversione del file tramite il servizio di '+terria.appName+'? Dovrebbe funzionare per ' +
            'piccoli, compressi Esri Shapefiles o MapInfo TAB files.')
    ...
    if (dataType === 'auto') {
        ...
        return getConfirmation(terria, viewState, confirmConversion, 'Tentare la conversione del file tramite il servizio di ' + terria.appName + '?')
            .then(function(confirmed) {
