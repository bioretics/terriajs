...
import { type ChartResult } from "../../Core/AttributeTable/chartSpec";
...
function HistogramChart(props: {
  ...
  if (!result) return <Empty>{t("attributeTable.chart.emptyNumeric")}</Empty>;
...
function ScatterChart(props: {
  ...
  if (!result) return <Empty>{t("attributeTable.chart.emptyScatter")}</Empty>;
...
function BarChart(props: {
  ...
  if (!result) return <Empty>{t("attributeTable.chart.emptyBar")}</Empty>;
  ...
  return (
    <>
      ...
      <Caption>
        {t(`attributeTable.chart.captionBar.${aggregation}`)}
        {truncated > 0
  ...
  return (
    <>
      ...
      >
        ...
        {axisTitle(t("attributeTable.chart.featureOrder"), colors.tick)}
      </ChartFrame>
      <Caption>
        {t("attributeTable.chart.captionLine", { count: points.length, field })}
      </Caption>
    </>
  );
}
...
function BoxChart(props: { result: BoxResult | undefined; field: string }) {
  ...
  return (
    <>
      ...
      <Caption>{t("attributeTable.chart.captionBox", { count })}</Caption>
    </>
  );
}
...
function PieChart(props: {
  ...
  const colors = useChartColors();
  const { result, aggregation, category } = props;
  if (!result) return <Empty>{t("attributeTable.chart.emptyPie")}</Empty>;
  ...
  return (
    <>
      ...
      <Caption>
        {t(
          aggregation === "count"
            ? "attributeTable.chart.captionPieCount"
            : "attributeTable.chart.captionPieSum",
          { count: slices.length }
        )}
      </Caption>
    </>
