...
import { type ChartRow } from "../../Core/AttributeTable/attributeCharts";
import {
  computeFieldStats,
  formatStatValue,
  resolveStatsScope,
  ...
  type FieldStats,
  type StatsScope
} from "../../Core/AttributeTable/attributeStats";
...
const FieldStatisticsDialog: React.FC<PropsType> = (props) => {
  ...
  // while the dialog is open, so the picker never points at an option that is
  // no longer offered.
  useEffect(() => {
  ...
  const stats = useMemo<FieldStats | undefined>(
    () => (field ? computeFieldStats(scopedRows, field) : undefined),
    [field, scopedRows]
  );
