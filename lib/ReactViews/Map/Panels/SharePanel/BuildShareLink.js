 ...
 */
export function buildShareLink(terria, returnRejected = false) {
    ...
    const _uri = uri.fragment(uri.query()).query('').toString(); // replace ? with #
    if(!returnRejected) {
        return _uri;
    }
    else{
        return {uri: _uri, rejected: _rejected};
    }
}
...
function getShareData(terria) {
    ...
    const rejected = addUserAddedCatalog(terria, initSources);
    ...
    _rejected = rejected.map(function(obj){
        return obj.name
    }).join(", ");
