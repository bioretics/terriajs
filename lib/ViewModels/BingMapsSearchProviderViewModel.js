...
var BingMapsSearchProviderViewModel = function(options) {
  ...
  if (!this.key) {
    console.warn(
      "Il geocodificatore " +
        this.name +
        " non ritorna riultati perchè manca la Bing Maps key. Ottenere una chiave sul sito bingmapsportal.com ed impostarla tramite parameters.bingMapsKey nel file config.json."
    );
  }
};
  ...
  var geocodeInProgress = (this._geocodeInProgress = promise
    .then(function(result) {
      ...
      if (result.resourceSets.length === 0) {
        that.searchMessage = "Spiacenti, la ricerca non ha dato risultati.";
        return;
      }
      ...
      for (var i = 0; i < resourceSet.resources.length; ++i) {
        ...
        const boundingBoxSize = 0.004;
        if(resource.bbox[3] - resource.bbox[1] < boundingBoxSize || resource.bbox[2] - resource.bbox[0] < boundingBoxSize) {
          resource.bbox[0] = parseFloat(resource.point.coordinates[0]) - boundingBoxSize * 0.5;
          resource.bbox[1] = parseFloat(resource.point.coordinates[1]) - boundingBoxSize * 0.5;
          resource.bbox[2] = parseFloat(resource.point.coordinates[0]) + boundingBoxSize * 0.5;
          resource.bbox[3] = parseFloat(resource.point.coordinates[1]) + boundingBoxSize * 0.5;
        }
    ...
    .otherwise(function() {
      ...
      that.isSearching = false;
      that.searchMessage =
        "La ricerca ha dato un errore. Verificare che sia presente la connessione internet e riprovare.";
    }));
