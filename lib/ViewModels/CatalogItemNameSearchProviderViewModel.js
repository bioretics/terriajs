...
CatalogItemNameSearchProviderViewModel.prototype.search = function(searchText) {
  ...
  return when(promise).then(function() {
    ...
    if (that.searchResults.length === 0) {
      that.searchMessage = "Spiacenti, la ricerca nel catalogo non ha dato risultati.";
    }
  });
};
...
function findMatchingItemByInfo(
  infos,
  searchExpression
)
{
  ...
  {
    if(searchExpression.test(infos[i].content))
      return true;
  }
  return false;
}
...
function findMatchingItemsRecursively(
  ...
  for (
    ...
    if (
      ...
      // Added search in catalog metadata
      //searchExpression.test(item.description)) &&
      findMatchingItemByInfo(item.info, searchExpression)) &&
