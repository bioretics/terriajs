    ...
    if ((terria.viewerMode === ViewerMode.CesiumTerrain || terria.viewerMode === ViewerMode.CesiumEllipsoid) && !this._useWebGL) {
        if (this._slowWebGLAvailable) {
            terria.error.raiseEvent({
                title : 'Performance WebGL troppo basse',
                message : 'Il browser ha riportato un problema di performance nella visualizzazione 3D di '+terria.appName+', \
è quindi possibile che il sito sia limitato alla modalità 2D. Per la piena modalità 3D provare ad usare un browser differente, ad aggiornare il browser, \
o ad aggiornare i driver della scheda video.'
            });
        } else {
            terria.error.raiseEvent({
                title : 'WebGL non supportato',
                message : terria.appName+' necessità di un browser che supporti <a href="http://get.webgl.org" target="_blank">WebGL</a>, \
ovvero uno tra le ultime versioni di <a href="http://www.google.com/chrome" target="_blank">Google Chrome</a>, \
...
Questo browser non sembra supportare WebGL quindi saranno disabilitate le funzioni 3D. \
Se il browser supporta WebGL provare ad aggiornare la pagina o riavviare il browser.'
            });
        }
...
function changeViewer(viewer) {
    ...
    if (newMode === ViewerMode.Leaflet) {
        ...
        if (!viewer._useWebGL) {
            terria.error.raiseEvent({
                title : 'WebGL non supportato o troppo lento',
                message : '\
Il browser non è in grado di utilizzare la modalità 3D, perchè non suppora WebGL oppure perchè il browser ha riscontrato un\'estream lentezza di WebGL.  \
Prova con un browser differente, ad esempio con l\'ultima versione di  <a href="http://www.google.com/chrome" target="_blank">Google Chrome</a>, \
<a href="http://www.mozilla.org/firefox" target="_blank">Mozilla Firefox</a>, \
<a href="https://www.apple.com/au/osx/how-to-upgrade/" target="_blank">Apple Safari</a>, or \
<a href="http://www.microsoft.com/ie" target="_blank">Microsoft Internet Explorer</a>.  In alcuni casi potrebbe essere necessario \
aggiornare il driver della scheda video o il sistema operativo per attivare la modalità 3D in ' + terria.appName + '.'
            });
            ...
            if (newMode === ViewerMode.CesiumTerrain) {
                ...
                terria.analytics.logEvent('mapSettings', 'switchViewer', '2D inclinato');
...
9TXL0Y4OHwAAAABJRU5ErkJggg==';
    ...
    viewer.scene.globe.baseColor = Color.IVORY;
        ...
        if (viewer.scene.terrainProvider instanceof CesiumTerrainProvider) {
            ...
            if (!defined(that.TerrainMessageViewed)) {
                that.terria.error.raiseEvent({
                    title : 'Il server del Terreno non risponde',
                    message : '\
Il server del Terreno al momento non risponde. Si possono usare comunque le funzionalità di '+that.terria.appName+' \
ma non in modalità 3D. Riprovando fra qualche tempo il terreno potrebbe \
funzionare come atteso. Se il problema persiste scrivere all\'indirizzo \
        ...
        if (!defined(that.checkedStartupPerformance)) {
            ...
            that.monitor.lowFrameRate.addEventListener( function() {
                if (!that.terria.cesium.stoppedRendering) {
                    that.terria.error.raiseEvent({
                        title : 'Rilevate basse performance',
                        message : 'Sembra che il sistema sia in grado di far girare  ' + that.terria.appName + ' \
in modalità 3D, ma sta avendo significativi problemi di performance. \
Per questo passerà automaticamente alla modalità 2D. Sarà comunque impostare nuovamente la modalità 3D.'
                    });
                    runLater(function() {
