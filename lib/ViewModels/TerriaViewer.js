  ...
  if (
    ...
    if (this._slowWebGLAvailable) {
      terria.error.raiseEvent({
        ...
        message:
          "Il browser ha riportato un problema di performance nella visualizzazione 3D di " +
          terria.appName +
          ", \
      ...
      terria.error.raiseEvent({
        ...
        message:
          terria.appName +
          ' necessità di un browser che supporti <a href="http://get.webgl.org" target="_blank">WebGL</a>, \
...
aggiornare il driver della scheda video o il sistema operativo per attivare la modalità 3D in ' +
          terria.appName +
          "."
      });
    ...
    if (viewer.scene.terrainProvider instanceof CesiumTerrainProvider) {
      ...
      if (!defined(that.TerrainMessageViewed)) {
        that.terria.error.raiseEvent({
          ...
          message:
            "\
            Il server del Terreno al momento non risponde. Si possono usare comunque le funzionalità di " +
            that.terria.appName +
            " \
        ...
        that.monitor.lowFrameRate.addEventListener(function() {
          if (!that.terria.cesium.stoppedRendering) {
            that.terria.error.raiseEvent({
              ...
              message:
                "Sembra che il sistema sia in grado di far girare  " +
                that.terria.appName +
                " \
