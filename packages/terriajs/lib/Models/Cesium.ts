...
export default class Cesium extends GlobeOrMap {
  ...
  constructor(terriaViewer: TerriaViewer, container: string | HTMLElement) {
    ...
    this._disposeTerrainReaction = autorun(() => {
      ...
      this.scene.globe.translucency.enabled =
        this.terria.globeTranslucencyEnabled;
      this.scene.screenSpaceCameraController.enableCollisionDetection =
        this.terria.enableCollisionDetection;
  ...
  getCurrentCameraView(): CameraView {
    ...
    // Fork (rer3d): in Cesium SCENE2D mode the frustum is orthographic and
    // has no fovy/aspectRatio — fall back to the home camera view instead of
    // computing a degenerate rectangle.
          ...
          result.features = allFeatures.reduce(
            (resultFeaturesSoFar, imageryLayerFeatures, _i) => {
              ...
              if (
                ...
                // Fork (rer3d): skip splitter filtering when the picked
                // position cannot be projected to the screen (e.g. SCENE2D).
                if (screenPosition) {
                  const pickedSide =
