...
describe('UserDrawing', function() {
    ...
    it('ensures onPointClicked callback is called when point is picked by user', function() {
        var Callback = function() {
            ...
            var that = this;
            this.callback = function(pointEntities) { that.pointEntities = pointEntities.entities.values.length; };
        };
        ...
        pickedFeatures.pickPosition = new Cartesian3(-5088454.576893678, 465233.10329933715, -3804299.6786334896);
    ...
    it('ensures graphics are updated when points change', function() {
        ...
        var currentPointPos = currentPoint.position.getValue(terria.clock.currentTime);
        ...
        currentPointPos = lineEntity.polyline.positions.getValue(terria.clock.currentTime)[0];
        expect(currentPointPos.x).toEqual(x);
        expect(currentPointPos.y).toEqual(y);
        expect(currentPointPos.z).toEqual(z);
        ...
        newPointPos = lineEntity.polyline.positions.getValue(terria.clock.currentTime)[1];
        expect(newPointPos.x).toEqual(newX);
        expect(newPointPos.y).toEqual(newY);
        expect(newPointPos.z).toEqual(newZ);
    });
    ...
    it('ensures onCleanUp callback is called when clean up occurs', function() {
        var Callback = function() {
            this.called = false;
            var that = this;
            this.callback = function() { that.called = true; };
        };
        var callback = new Callback();
        var options = { terria: terria,
    ...
    it('function clickedExistingPoint detects and handles if existing point is clicked', function() {
        ...
        var pt1Position = new Cartographic(CesiumMath.toRadians(149.121), CesiumMath.toRadians(-35.309), CesiumMath.toRadians(0));
        var pt1CartesianPosition = Ellipsoid.WGS84.cartographicToCartesian(pt1Position);
        pickedFeatures.pickPosition = pt1CartesianPosition;
        pickedFeatures.allFeaturesAvailablePromise = true;
        userDrawing.terria.mapInteractionModeStack[0].pickedFeatures = pickedFeatures;

        // Second point
        var pt2Position = new Cartographic(CesiumMath.toRadians(149.124), CesiumMath.toRadians(-35.311), CesiumMath.toRadians(0));
        var pt2CartesianPosition = Ellipsoid.WGS84.cartographicToCartesian(pt2Position);
        pickedFeatures.pickPosition = pt2CartesianPosition;
        userDrawing.terria.mapInteractionModeStack[0].pickedFeatures = pickedFeatures;

        // Third point
        var pt3Position = new Cartographic(CesiumMath.toRadians(149.127), CesiumMath.toRadians(-35.308), CesiumMath.toRadians(0));
        var pt3CartesianPosition = Ellipsoid.WGS84.cartographicToCartesian(pt3Position);
    ...
    it('polygon is only drawn once', function() {
        ...
        var newPtPosition = new Cartographic(CesiumMath.toRadians(149.000), CesiumMath.toRadians(-35.000), CesiumMath.toRadians(0));
        var newPtCartesianPosition = Ellipsoid.WGS84.cartographicToCartesian(newPtPosition);
